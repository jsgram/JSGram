...
export const sendNewsEmail = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const templatePath = path.join(process.env.HEROKU_ROOT, process.env.TEMPLATE_DIR, 'news.email.pug');
        ...
        await Promise.all(users.map(async (user: IUserModel) => {
            const { body: { response: { compare, sender: { login, html_url } } } }: Request = req;
